...
const search = () => {
    router.get(route('inventario.metodo.ueps'), form.value, { preserveState: true });
};
...
const downloadPDF = () => {
    ...
    try {
        const doc = new jsPDF();
        doc.setFontSize(16);
        doc.text(`Kardex UEPS - ${props.selectedIngrediente?.nombre || 'Reporte'}`, 14, 15);
        doc.setFontSize(10);
        ...
        doc.save(`kardex_ueps_${props.selectedIngrediente?.nombre || 'reporte'}_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
        console.error('Error generando PDF UEPS:', err);
    ...
    <AppLayout title="Método UEPS">
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-amber-800 dark:text-amber-300 leading-tight">
                    <i class="fa-solid fa-sort-amount-up mr-2"></i> Método UEPS
                </h2>
            </div>
        </template>
        ...
        <div class="py-12 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 transition-colors duration-300">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div>
                            ...
                            <select v-model="form.ingrediente_id" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-amber-500">
                        ...
                        <div>
                            ...
                            <input type="date" v-model="form.fecha_inicio" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-amber-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hasta</label>
                            <input type="date" v-model="form.fecha_fin" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-amber-500">
                        </div>
                        <div class="flex gap-2">
                            <button @click="search" class="bg-amber-600 dark:bg-amber-700 text-white px-4 py-2 rounded-lg hover:bg-amber-700 dark:hover:bg-amber-600 flex-1 transition-colors duration-300">Consultar</button>
                ...
                <div v-if="selectedIngrediente" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700 transition-colors duration-300">
                    <div class="p-4 bg-amber-50 dark:bg-amber-900/30 border-b border-amber-100 dark:border-amber-800 flex justify-between items-center transition-colors duration-300">
                        <h3 class="font-bold text-amber-800 dark:text-amber-300">Kardex UEPS: {{ selectedIngrediente.nombre }}</h3>
                    </div>
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 uppercase font-medium transition-colors duration-300">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center bg-amber-50 dark:bg-amber-900/20 font-bold">Saldo</th>
                                    <th class="px-6 py-3 text-right bg-amber-50 dark:bg-amber-900/20 font-bold">Valor</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-colors duration-300">
                                <tr v-for="(row, idx) in kardex" :key="idx" 
                                    ...
                                    <td class="px-6 py-3 text-right bg-amber-50/50 dark:bg-amber-900/10 font-bold text-gray-900 dark:text-gray-100">Bs {{ Number(row.saldo_total).toFixed(2) }}</td>
                                </tr>
                                <tr v-if="kardex.length === 0">
                                    <td colspan="8" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                        <i class="fa-solid fa-info-circle text-4xl mb-2 text-amber-300 dark:text-amber-600"></i>
                                        <p class="font-medium">Sin movimientos para este ingrediente.</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div v-if="kardex.length > 0" class="bg-gradient-to-r from-amber-600 dark:from-amber-700 to-orange-600 dark:to-orange-700 p-6 text-white transition-colors duration-300">
                        <div class="grid grid-cols-2 gap-6">
                            <div class="text-center border-r border-white/30">
                                <p class="text-amber-100 dark:text-amber-200 text-sm uppercase tracking-wide mb-2">Existencia Total</p>
                                <p class="text-4xl font-bold">{{ existenciaActual }}</p>
                                <p class="text-amber-200 dark:text-amber-300 text-sm mt-1">{{ selectedIngrediente.unidad_medida }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-amber-100 dark:text-amber-200 text-sm uppercase tracking-wide mb-2">Valor de Inventario</p>
                                <p class="text-4xl font-bold">Bs {{ Number(valorInventario).toFixed(2) }}</p>
                                <p class="text-amber-200 dark:text-amber-300 text-sm mt-1">Según método UEPS</p>
                            </div>
                        </div>
                    </div>
                </div>
