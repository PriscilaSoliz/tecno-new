...
const search = () => {
    router.get(route('inventario.metodo.peps'), form.value, { preserveState: true });
};
...
const downloadPDF = () => {
    ...
    try {
        const doc = new jsPDF();
        doc.setFontSize(16);
        doc.text(`Kardex PEPS - ${props.selectedIngrediente?.nombre || 'Reporte'}`, 14, 15);
        doc.setFontSize(10);
        ...
        const tableData = props.kardex.map(k => [
            ...
            k.tipo,
            String(k.cantidad),
            'Bs ' + Number(k.precio_unitario).toFixed(2),
        ...
        const tableOptions = {
            head: [['Fecha', 'Mov.', 'Cant', 'P. Unit', 'Total', 'Exist.', 'Saldo Q', 'Saldo $']],
            body: tableData,
            startY: 35,
        ...
        doc.save(`kardex_peps_${props.selectedIngrediente?.nombre || 'reporte'}_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
        console.error('Error generando PDF PEPS:', err);
    ...
    <AppLayout title="Método PEPS">
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-emerald-800 dark:text-emerald-300 leading-tight">
                    <i class="fa-solid fa-sort-amount-down mr-2"></i> Método PEPS
                </h2>
            </div>
        </template>
        ...
        <div class="py-12 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 transition-colors duration-300">
                    ...
                    <!-- Filtros -->
                    <div class="bg-white dark:bg-gray-700 rounded-xl shadow-sm p-6 mb-6 border-t-4 border-emerald-500 transition-colors duration-300">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Ingrediente</label>
                                <select v-model="form.ingrediente_id" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <div>
                                ...
                                <input type="date" v-model="form.fecha_inicio" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-emerald-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hasta</label>
                                <input type="date" v-model="form.fecha_fin" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:ring-emerald-500">
                            </div>
                            <div class="flex gap-2">
                                <button @click="search" class="bg-emerald-600 dark:bg-emerald-700 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 dark:hover:bg-emerald-600 flex-1 transition-colors duration-300">
                                    <i class="fa-solid fa-search mr-2"></i> Consultar
                                </button>
                                <button v-if="kardex.length" @click="downloadPDF" class="bg-amber-600 dark:bg-amber-700 text-white px-4 py-2 rounded-lg hover:bg-amber-700 dark:hover:bg-amber-600 transition-colors duration-300">
                    ...
                    <div v-if="selectedIngrediente" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700 transition-colors duration-300">
                        <div class="p-4 bg-emerald-50 dark:bg-emerald-900/30 border-b border-emerald-100 dark:border-emerald-800 flex justify-between items-center transition-colors duration-300">
                            <h3 class="font-bold text-emerald-800 dark:text-emerald-300">Kardex PEPS: {{ selectedIngrediente.nombre }}</h3>
                            <span class="text-sm text-emerald-600 dark:text-emerald-400">Unidad: {{ selectedIngrediente.unidad_medida }}</span>
                        </div>
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 uppercase font-medium transition-colors duration-300">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-center bg-green-50 dark:bg-green-900/20">Entradas / Salidas</th>
                                        <th class="px-6 py-3 text-right">Costo Unit.</th>
                                        <th class="px-6 py-3 text-right">Total Mov.</th>
                                        <th class="px-6 py-3 text-center bg-blue-50 dark:bg-blue-900/20 font-bold">Exist. Lote</th>
                                        <th class="px-6 py-3 text-center bg-amber-50 dark:bg-amber-900/20 font-bold">Saldo Cant.</th>
                                        <th class="px-6 py-3 text-right bg-amber-50 dark:bg-amber-900/20 font-bold">Saldo Valor</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-700 transition-colors duration-300">
                                    <tr v-for="(row, idx) in kardex" :key="idx" 
                                        ...
                                        <td class="px-6 py-3">
                                            <span :class="row.tipo === 'ENTRADA' ? 'text-green-700 dark:text-green-400 font-bold' : 'text-red-700 dark:text-red-400 font-bold'">
                                                {{ row.tipo }}
                                            </span>
                                            <span class="text-xs text-gray-500 dark:text-gray-400 block">{{ row.motivo }}</span>
                                        </td>
                                        <td class="px-6 py-3 text-center font-bold bg-green-50/50 dark:bg-green-900/10" 
                        ...
                        <!-- Totales Finales -->
                        <div v-if="kardex.length > 0" class="bg-gradient-to-r from-emerald-600 dark:from-emerald-700 to-green-600 dark:to-green-700 p-6 text-white transition-colors duration-300">
                            <div class="grid grid-cols-2 gap-6">
                                <div class="text-center border-r border-white/30">
                                    <p class="text-emerald-100 dark:text-emerald-200 text-sm uppercase tracking-wide mb-2">Existencia Total Actual</p>
                                    <p class="text-4xl font-bold">{{ existenciaActual }}</p>
                                    <p class="text-emerald-200 dark:text-emerald-300 text-sm mt-1">{{ selectedIngrediente.unidad_medida }}</p>
                                </div>
                    ...
                    <!-- Mensaje cuando no hay ingrediente -->
                    <div v-else class="text-center py-16 bg-white dark:bg-gray-700 rounded-xl shadow-sm transition-colors duration-300">
                        <i class="fa-solid fa-arrow-up text-6xl mb-4 text-emerald-300 dark:text-emerald-600"></i>
                        <p class="text-lg text-gray-600 dark:text-gray-300 font-medium">Seleccione un ingrediente para ver su Kardex PEPS</p>
                        <p class="text-sm text-gray-400 dark:text-gray-500 mt-2">El sistema calculará automáticamente entradas y salidas.</p>
                    </div>
                </div>
            </div>
        </div>
